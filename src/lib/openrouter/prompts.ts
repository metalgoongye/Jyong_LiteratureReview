...

{
  ...
  "sections": [
    {
      "section_type": "research_background" | "research_question" | "methodology" | "findings" | "conclusion" | "literature_review" | "theoretical_framework" | "data_sources" | "limitations" | "other",
...
3. sections — section_type assignment rules:
   - "research_background": MUST be the FIRST section (section_order: 1) if the paper has an introduction or background. Include here ONLY statistics and empirical findings that THIS paper CITES FROM OTHER WORKS to motivate the study (e.g., "According to UN (2023), urban areas account for 70% of CO2 emissions"). Each bullet should note the cited source in parentheses. This section represents the evidential backdrop the authors build their research question upon.
   - "research_question": the gap, objective, or hypothesis the authors themselves define — comes AFTER research_background.
   - All other section types follow naturally after research_question.
4. Empirical evidence: Extract ONLY quantitative findings produced by THIS paper's own analysis — results from the authors' own data collection, regression, SEM, survey, or experiment reported in the Results/Findings sections. STRICTLY EXCLUDE: (a) statistics cited from other studies (those belong in research_background), (b) background statistics used to motivate the study, (c) descriptive statistics of the sample unless they are a primary finding. Pay careful attention to TABLES and FIGURES — read regression coefficients, β values, odds ratios, p-values, R², and model fit statistics directly from table cells and figure annotations. Do NOT rely on the text narrative alone for numbers. If the abstract reports key numerical findings, cross-check for consistency — but do NOT require abstract confirmation (some papers omit numbers in abstracts). If no qualifying finding exists, return empirical_evidence as an empty array []. metric_value MUST contain the actual number. evidence_text in English (one sentence clearly attributing the finding to this paper's own analysis), evidence_text_korean in Korean (학술체). page_reference: only if CERTAIN from [PAGE X] markers, otherwise null. Max 5 items.
5. Bullets must be 개조식 — concise phrases, NOT full sentences. Max 4 bullets per section. Max 6 sections total (including research_background). Keep JSON output compact.
