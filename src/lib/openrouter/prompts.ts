...
3. Empirical evidence: Extract ONLY quantitative findings produced by THIS paper's own analysis — i.e., results from the authors' own data collection, regression, SEM, survey, or experiment reported in the Results/Findings sections. STRICTLY EXCLUDE: (a) statistics cited from other studies in the Introduction or Literature Review (e.g., "Smith (2020) found that 30%..."), (b) background statistics used to motivate the study, (c) descriptive statistics of the sample unless they are the main finding. If the abstract reports key numerical findings, cross-check that each evidence item matches or is consistent with the abstract — but do NOT require abstract confirmation to include an item (some papers omit numbers in abstracts). If no qualifying finding exists, return empirical_evidence as an empty array []. metric_value MUST contain the actual number. evidence_text in English (one sentence with the number included, clearly attributing it to this paper's own analysis), evidence_text_korean in Korean (학술체). page_reference: only if CERTAIN from [PAGE X] markers, otherwise null. Max 5 items.
